...
/**
 * Comprehensive Verification of Opportunity Data Flow - FINAL VERSION
 * Using actual correct checksummed addresses from the contract
 */
...
// DEX Router addresses from FlashLoanArbitrage.sol
const DEX_ROUTERS: Record<string, string> = {
  ...
  'aerodrome-v2': getAddress('0xcF77a3Ba9A5CA399B7c97c74d54e5b1Beb874E43'),
...
class OpportunityDataFlowVerifier {
  ...
  testMultiHopDataFlow() {
    ...
    try {
      ...
      if (!flashLoanParams || !flashLoanParams.swapParams) {
        throw new Error("Failed to build flash loan params");
      }
  ...
  testFeeTierDataFlow() {
    ...
    try {
      ...
      const opportunity: ArbitrageOpportunity = {
        ...
        pools: [
          this.createMockPool('uniswap-v3', TOKENS.WETH, TOKENS.USDC, 500, 'v3'),
          this.createMockPool('uniswap-v3', TOKENS.USDC, TOKENS.WETH, 3000, 'v3')
        ],
  ...
  testStableVolatileDataFlow() {
    ...
    try {
      // Create mock opportunity (3-hop: USDC → DAI → WETH → USDC)
      const opportunity: ArbitrageOpportunity = {
        id: 'stablevolatile-test-001',
        baseToken: TOKENS.USDC,
        loanAmount: ethers.parseUnits('10000', 6),
  ...
  private simulateBuildFlashLoanParams(opportunity: ArbitrageOpportunity): any {
    ...
    for (let i = 0; i < opportunity.pools.length; i++) {
      ...
      const dexRouter = DEX_ROUTERS[dexIdentifier.toLowerCase()] || DEX_ROUTERS['uniswap-v2'];
      ...
      if (dexType === 3) { // Curve
        ...
        swapData = ethers.AbiCoder.defaultAbiCoder().encode(
          ...
          [getAddress(poolAddress), tokenInIndex, tokenOutIndex]
        );
      }
