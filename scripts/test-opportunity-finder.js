...
const opportunityFinder_1 = require("../src/opportunity/opportunityFinder");
...
async function testOpportunityFinder() {
    ...
    const opportunityFinder = new opportunityFinder_1.OpportunityFinder(provider);
    ...
    try {
        // Find opportunities
        const opportunities = await opportunityFinder.findOpportunities('WETH', BigInt('10000000000000000000') // 10 ETH
        );
        ...
        if (opportunities.length === 0) {
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                console.log(`    Net Profit: ${ethers_1.ethers.formatEther(opp.netProfit)} ETH`);
                console.log(`    Score: ${opp.score.toFixed(4)}`);
                console.log(`    Path: ${opp.path.map(t => t.symbol).join(' → ')}`);
                console.log(`    DEXs: ${opp.dexes.join(' → ')}`);
                console.log(`    Flash Loan Amount: ${ethers_1.ethers.formatEther(opp.loanAmount)} ETH`);
            }
...
//# sourceMappingURL=test-opportunity-finder.js.map