/**
 * BaseSwap Pool Fetcher
 * Fetches all V2-style pairs from BaseSwap using multicall
 * 
 * BaseSwap uses the same architecture as Uniswap V2:
 * - Factory contract with allPairs() and getPair()
 * - Pair contracts with getReserves()
...
// BaseSwap Factory ABI (same as Uniswap V2)
const BASESWAP_FACTORY_ABI = [
...
// BaseSwap Pair ABI (same as Uniswap V2)
const BASESWAP_PAIR_ABI = [
...
export class BaseSwapFetcher {
  ...
  constructor(provider: ethers.Provider, config: DEXConfig) {
    ...
    if (!config.factory) {
      throw new Error(`BaseSwap factory address is required for ${config.name}`);
    }
    
    this.factory = new ethers.Contract(config.factory, BASESWAP_FACTORY_ABI, provider);
  }
  ...
  async fetchAllPools(baseToken?: string): Promise<PoolFetchResult> {
    ...
    try {
      console.log(`Fetching BaseSwap pairs from factory: ${this.config.factory}`);
      ...
      console.log(`BaseSwap fetch complete: ${filteredPools.length} pairs (${stats.executionTime}ms)`);
      ...
      stats.executionTime = Date.now() - startTime;
      console.error('Fatal error in BaseSwapFetcher:', error);
  ...
  private async fetchPairStates(pairAddresses: string[]): Promise<Pool[]> {
    ...
    const pairInterface = new ethers.Interface(BASESWAP_PAIR_ABI);
