/**
 * Aerodrome SlipStream 2 Pool Fetcher
 * Fetches all V3-style CL AMM pools from Aerodrome SlipStream 2
 * 
 * Aerodrome SlipStream 2 uses the same architecture as Aerodrome SlipStream and Uniswap V3:
 * - Factory contract with PoolCreated events
 * - Pool contracts with slot0() and liquidity()
 * - V3-style concentrated liquidity AMM
 */
...
// Aerodrome SlipStream 2 Factory ABI (same as Uniswap V3)
const AERODROME_SLIPSTREAM_2_FACTORY_ABI = [
  'event PoolCreated(address indexed token0, address indexed token1, uint24 indexed fee, int24 tickSpacing, address pool)',
  'function getPool(address tokenA, address tokenB, uint24 fee) external view returns (address pool)',
];

// Aerodrome SlipStream 2 Pool ABI (same as Uniswap V3)
const AERODROME_SLIPSTREAM_2_POOL_ABI = [
...
export class AerodromeSlipStream2Fetcher {
  ...
  constructor(provider: ethers.Provider, config: DEXConfig) {
    ...
    if (!config.factory) {
      throw new Error(`Aerodrome SlipStream 2 factory address is required for ${config.name}`);
    }
    
    this.factory = new ethers.Contract(config.factory, AERODROME_SLIPSTREAM_2_FACTORY_ABI, provider);
  }
  ...
  async fetchAllPools(baseToken?: string): Promise<PoolFetchResult> {
    ...
    try {
      console.log(`Fetching Aerodrome SlipStream 2 pools from factory: ${this.config.factory}`);
      ...
      console.log(`Aerodrome SlipStream 2 fetch complete: ${filteredPools.length} pools (${stats.executionTime}ms)`);
      ...
      console.error('Fatal error in AerodromeSlipStream2Fetcher:', error);
  ...
  private async fetchPoolStates(poolInfos: Array<{
    ...
    const poolInterface = new ethers.Interface(AERODROME_SLIPSTREAM_2_POOL_ABI);
    ...
    for (let i = 0; i < poolInfos.length; i++) {
      ...
      try {
        ...
        const pool: Pool = {
          ...
          dex: this.config.dexId || this.config.name.toLowerCase().replace(/\s+/g, '-'),
