/**
 * Aerodrome SlipStream Pool Fetcher
 * Fetches all V3-style CL AMM pools from Aerodrome SlipStream
 * 
 * Aerodrome SlipStream uses the same architecture as Uniswap V3:
...
// Aerodrome SlipStream Factory ABI (same as Uniswap V3)
const AERODROME_SLIPSTREAM_FACTORY_ABI = [
  'event PoolCreated(address indexed token0, address indexed token1, uint24 indexed fee, int24 tickSpacing, address pool)',
  'function getPool(address tokenA, address tokenB, uint24 fee) external view returns (address pool)',
];

// Aerodrome SlipStream Pool ABI (same as Uniswap V3)
...
export class AerodromeSlipStreamFetcher {
  ...
  constructor(provider: ethers.Provider, config: DEXConfig) {
    ...
    if (!config.factory) {
      throw new Error(`Aerodrome SlipStream factory address is required for ${config.name}`);
    }
    
    this.factory = new ethers.Contract(config.factory, AERODROME_SLIPSTREAM_FACTORY_ABI, provider);
  }
  ...
  async fetchAllPools(baseToken?: string): Promise<PoolFetchResult> {
    ...
    try {
      console.log(`Fetching Aerodrome SlipStream pools from factory: ${this.config.factory}`);
      ...
      console.log(`Aerodrome SlipStream fetch complete: ${filteredPools.length} pools (${stats.executionTime}ms)`);
      ...
      console.error('Fatal error in AerodromeSlipStreamFetcher:', error);
  ...
  private async fetchPoolStates(poolInfos: Array<{
    ...
    const poolInterface = new ethers.Interface(AERODROME_SLIPSTREAM_POOL_ABI);
