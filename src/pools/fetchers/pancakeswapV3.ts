/**
 * PancakeSwap V3 Pool Fetcher
 * Fetches all V3 pools from PancakeSwap V3 Factory (uses Uniswap V3 interface)
 */
...
export class PancakeSwapV3Fetcher {
  ...
  async fetchAllPools(baseToken?: string): Promise<PoolFetchResult> {
    ...
    try {
      console.log(`Fetching PancakeSwap V3 pools from factory: ${this.config.factory}`);
      ...
      console.log(`PancakeSwap V3 fetch complete: ${filteredPools.length} pools (${stats.executionTime}ms)`);
      ...
      console.error('Fatal error in PancakeSwapV3Fetcher:', error);
  ...
  private async fetchPoolStates(poolInfos: any[]): Promise<Pool[]> {
    ...
    const poolInterface = new ethers.Interface(PANCAKESWAP_V3_POOL_ABI);
